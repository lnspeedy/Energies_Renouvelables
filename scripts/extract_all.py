...
def download_source(source_name, details):
    try:
        ...
        response = requests.get(details["url"], headers=HEADERS, timeout=90, stream=True)
        ...
        if details["filename"].endswith('.zip'):
            ...
            with zipfile.ZipFile(io.BytesIO(response.content)) as z:
                csv_files = [f for f in z.infolist() if f.filename.lower().endswith('.csv') and not f.is_dir()]
                if not csv_files: raise FileNotFoundError("Aucun fichier CSV trouvé dans le ZIP.")
                main_csv = max(csv_files, key=lambda f: f.file_size)
                print(f"     -> Fichier principal détecté : '{main_csv.filename}'")
                # On sauvegarde le CSV extrait sous un nom prédictible
                with z.open(main_csv) as source, open(RAW_DATA_PATH / f"{source_name}.csv", 'wb') as target:
...
def extract_financial_data():
    ...
    COMPANIES = {
        ...
        "FSLR": "First Solar", "ORA.PA": "Orsted", "ENPH": "Enphase Energy"
    }
    try:
