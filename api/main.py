...
def get_available_sources():
    if not PROCESSED_DATA_PATH.exists(): return []
    return sorted([d.name for d in PROCESSED_DATA_PATH.iterdir() if d.is_dir()])

# --- Catalogue de Métadonnées (inchangé) ---
METADATA_CATALOG = {
    "rte": {
        "ID_Source": "FR-RTE-01", "Nom_Source": "Réseau de Transport d'Électricité (RTE)",
        "Producteur": "RTE", "Nom_Jeu_Donnees": "eCO2mix - Données nationales temps réel",
        ...
        "Unites": "MW", "Couverture_Temporelle": "Depuis 2012", "Format_Fichier": "CSV",
        ...
        "Acces_API": "Oui (Opendatasoft API v2)", "Frequence_Maj": "Temps réel", "Date_Derniere_Maj": "Continue",
        "Personnes_En_Charge": "Service Open Data RTE"
    },
    "world_bank": {
        "ID_Source": "INT-WB-01", "Nom_Source": "La Banque Mondiale", "Producteur": "The World Bank Group",
        ...
        "Unites": "%", "Couverture_Temporelle": "Annuelle, depuis 1990", "Format_Fichier": "XLS",
        "Methode_Recuperation": "Téléchargement direct via portail de données", "Lien_Acces": "https://data.worldbank.org/indicator/EG.ELC.RNWX.ZS",
        "Acces_API": "Oui (World Bank Data API)", "Frequence_Maj": "Annuelle", "Date_Derniere_Maj": "Annuelle",
        "Personnes_En_Charge": "World Bank Data Group"
    },
    "stock_prices": {
        "ID_Source": "FIN-YFIN-01", "Nom_Source": "Yahoo Finance", "Producteur": "Yahoo!",
        ...
        "Unites": "Devise locale de la bourse", "Couverture_Temporelle": "Historique sur 5 ans", "Format_Fichier": "CSV (via librairie)",
        "Methode_Recuperation": "Librairie Python `yfinance`", "Lien_Acces": "https://finance.yahoo.com/",
        "Acces_API": "Non officiel (via `yfinance`)", "Frequence_Maj": "Quotidienne", "Date_Derniere_Maj": "Continue",
...
# --- Endpoints de l'API ---
@app.get("/")
def read_root(): return {"message": "API Analytique des Énergies Renouvelables"}

@app.get("/sources_with_metadata", summary="Lister toutes les sources avec leurs métadonnées")
def list_sources_with_metadata():
    available_sources = get_available_sources()
    response_data = {source: METADATA_CATALOG.get(source, {"Nom_Source": source, "Description": "Aucune métadonnée disponible."}) for source in available_sources}
...
def get_data_by_source(
    ...
    try:
        ...
        if q:
            ...
            text_search_cols = ['title', 'abstract', 'nom_politique', 'nom_centrale']
            df_cols = con.execute(f"DESCRIBE SELECT * FROM read_parquet('{parquet_glob_path}')").fetchdf()['column_name'].tolist()
            ...
            if searchable_cols:
                ...
                for _ in searchable_cols: params.append(f'%{q}%')
        ...
        return {"source": source_name, "count": len(df_cleaned), "data": df_cleaned.to_dict("records")}
        
    except Exception as e:
        import traceback
        print(f"Erreur pour la source '{source_name}':")
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Erreur interne du serveur pour la source '{source_name}': {e}")